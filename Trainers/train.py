...
import json
from utils.utils import save_samples, parse_arguments, make_path, get_all_audio_filepaths, split_data, time_since, \
...
for epoch in range(1, epochs+1):
    ...
    for i in range(1, BATCH_NUM+1):
        ...
        for iter_dis in range(5):
            ...
            real_data_Var = numpy_to_var(next(train_iter)['X'], device)
            ...
            fake = autograd.Variable(netG(noise).data)  # noise_Var
            D_fake = netD(fake)
            D_fake = D_fake.mean()
            D_fake.backward(one)
            ...
            valid_data_Var = numpy_to_var(next(valid_iter)['X'], device)
            D_real_valid = netD(valid_data_Var)
            D_real_valid = D_real_valid.mean()  # avg loss

            # b) compute loss contribution from generated data, then backprop.
            fake_valid = netG(noise)  # noise_Var
