...
def uttname2spkid(uttname): return uttname.split('_')[0]


def denormalize_wave_minmax(x): return (65535. * x / 2) - 1 + 32767.
...
def composite_helper(args): return eval_composite(*args)
...
class ComposeAdditive(object):
    ...
    def __call__(self, x): return x, self.additive(x)


class Additive(object):
    def __init__(self, noises_dir, snr_levels=None, do_IRS=False):
