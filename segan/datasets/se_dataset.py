...
from segan.datasets.data_tools import interpolation
from segan.datasets.data_tools import read_aco_file
...
def slice_signal_index(path, window_size, stride):
    ...
    """
    signal, rate = librosa.load(path, 16000)  # opts.slice_size  # 16000
...
def pre_emphasize(x, coef=0.95):
    ...
    x0 = np.reshape(x[0], (1,))
    diff = x[1:] - coef * x[:-1]
    return np.concatenate((x0, diff), axis=0)
...
class SEDataset(Dataset):
    ...
    def __init__(self, clean_dir, noisy_dir, preemph, cache_dir='.', split='train', slice_size=2 ** 14,
                 stride=0.5, max_samples=None, do_cache=False, verbose=False,
                 slice_workers=2, preemph_norm=False, random_scale=[1]):
...
class RandomChunkSEDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        slice_size = min_L if self.slice_size > min_L else self.slice_size
...
class SEH5Dataset(Dataset):
    ...
    def __init__(self, data_root, split, preemph,
                 ...
                 preemph_norm=False,
                 random_scale=[1]):
        super(SEH5Dataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        return ['N/A', torch.FloatTensor(c_slice).squeeze(-1),
